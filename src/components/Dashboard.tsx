...
const Dashboard: FC<DashboardProps> = ({ role }): React.ReactElement => {
        ...
        try {
            ...
            const tokenPromises = tokenIds.map(async (tokenId) => {
                try {
                    ...
                    if (role === 'productor' || (role === 'fabrica' && !isProcesado)) {
    ...
    const filteredTokens = useCallback(() => {
        if (role === 'fabrica') {
            return tokens.filter(token => !token.isProcesado);
        }
        // Para productor y otros roles, mostrar todos los tokens
        return tokens;
    }, [tokens, role]);

    // Función para obtener los tokens procesados (solo para fábrica)
    const processedTokens = useCallback(() => {
        if (role === 'fabrica') {
            return tokens.filter(token => token.isProcesado);
        }
        return [];
            ...
            <div className="w-full px-4 sm:px-6 lg:px-8 py-8">
                ...
                {loading ? (
                    ...
                    <div className="space-y-8">
                        ...
                        {filteredTokens().length > 0 ? (
                            <div className="overflow-hidden shadow ring-1 ring-black ring-opacity-5 sm:rounded-lg">
                                <h3 className="text-xl font-semibold text-gray-800 p-4 bg-white border-b">
                                    {role === 'fabrica' ? 'Materias Primas' : 'Mis Productos'}
                                </h3>
                        ...
                        {/* Tabla de productos procesados - solo para fábrica */}
                        {role === 'fabrica' && processedTokens().length > 0 && (
                            <div className="mt-8">
